...
for number in tqdm(bingo_numbers):
  ...
  for card_inx, card in enumerate(bingo_cards):
    if card_inx in won_cards_inx:
      LOGGER.log(f"Skipping card {card_inx}: Already won")
      continue
  ...
  for card_inx, card in enumerate(bingo_cards):
    ...
    if check_win(card):
      score: int = sum_card(card, number)
      LOGGER.log(f"Card {card_inx} won. Score: {score}")
      won_cards_inx.append(card_inx)
...
LOGGER.tee(f"Last winning score: {score}")
LOGGER.dump_log()
