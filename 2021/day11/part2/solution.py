...
for inline in inlines:
    ...
    for c in inline:
        l.append(int(c))
    ingrid.append(l)
OCTO_COUNT: int = sum([len(l) for l in ingrid])
LOGGER.tee(f"Successfully read {len(ingrid)} lines with {OCTO_COUNT} numbers.")
...
while not all_flashed:
    stepinx += 1
    LOGGER.log(f"Starting step {stepinx+1}")
    LOGGER.indent()
    stepflashes: [(int, int)] = []
    flashqueue: [(int, int)] = []
    ...
    LOGGER.unindent()
    if len(stepflashes) == OCTO_COUNT:
        all_flashed = True
LOGGER.unindent()
LOGGER.tee(f"SOLVE: The first step with all octopusses flashing is step {stepinx}.")
