    ...
    def __init__(self, start: Coordinate, end: Coordinate):
        ...
        if start.x == end.x and start.y < end.y:
            ...
            for x_offset in x_offsets:
                ...
                coord: Coordinate = Coordinate(x, start.y)
                self.all_coords.append(coord)
        elif start.y < end.y and start.x < end.x:  # top-left -> bot-right
            diff: int = end.y - start.y
            for offset in range(diff+1):
                y = start.y + offset
                x = start.x + offset
                coord: Coordinate = Coordinate(x, y)
                self.all_coords.append(coord)
        elif start.y < end.y and start.x > end.x:  # top-right -> bot-left
            diff: int = end.y - start.y
            for offset in range(diff+1):
                y = start.y + offset
                x = start.x - offset
                coord: Coordinate = Coordinate(x, y)
                self.all_coords.append(coord)
        elif start.y > end.y and start.x < end.x:  # bot-left -> top-right
            diff: int = start.y - end.y
            for offset in range(diff+1):
                y = start.y - offset
                x = start.x + offset
                coord: Coordinate = Coordinate(x, y)
                self.all_coords.append(coord)
        elif start.y > end.y and start.x > end.x:  # bot-right -> top-left
            diff: int = start.y - end.y
            for offset in range(diff+1):
                y = start.y - offset
                x = start.x - offset
